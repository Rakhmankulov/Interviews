...
`CMD` и `ENTRYPOINT` - это почти одни и те же команды. `CMD` это какие-то команды, которые выполняюся при запуске контейнера и их можно перезаписать при запуске контейнера. А `ENTRYPOINT` нельзя перезаписать и, то есть то что бывает записывают там что-то entry поим что-то `CMD` и то что `ENTRYPOINT` там у тебя ты не можешь переписать у тебя это ну  

у тебя автоматически идёт с запись команда, а потом уже кото ты можеш переть то что теб при ничего не конфликтует и так далее 
