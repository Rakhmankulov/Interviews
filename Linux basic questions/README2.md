...
Ну что это Это количество процессов и операций ввода-вывода если я не ошибаюсь за там 1 минуту 5 минут и 15 минут которые находятся в ожидании процессного времени вот а какое оптимальное наме число будет вот этих едини при че ядрах три я думаю ну когда четыре это значит что скорее всего там чуть ли не под завязку забит А вот когда тамри условно е там
01:43
работать ещ может Ну да уже так он так себе будет работать скорее всего так что вот там три Ну вот там 8 80 75% Вот вот я думаю оптимально дальше там надо уже что-то решать воз оптимизировать чтото такое а тогда такой вот кейс к тебе разработчик подходит то есть есть одна виртуалка да на ней четыре ядра к тебе подходит разработчик говорит там на ней база данных   развёрнутая там ну в два раза больше если тебя четыре ядра у тебя там во Да уже вот но ты смотришь на CPU проценты Угу лишь 20-30 то есть пробле что ещ Может
02:33
быть куда можно копать Ну потому что как знаю может считаться не только по CP нагрузке там може там нагрузка там шина данных там вот нагрузка операции ввода-вывода как раз таки и я бы уже копал там возможно влоги вот самих ти баз данных там как там вообще сервисы развёрнуты как они там настроены Как там вот этот Возможно нарот и так далее я бы уже вот в этом плане возможно Там гдето слишком много каких-то времен файлов которые постоянно создаются Возможно или что-то такое Ну вот я бы уже вот в этом плане капал там
03:06
возможно Ну да вот О'кей А можешь сказать например э ну такой простой вопрос вот как процесс найти например мне ну с помощью топа с помощью там PS мину и уже по пиду там уже искать его либо подут там убивать и так далее Вот вот основное А смотри вот ты нашёл например процесс Да допустим Уде всю информацию о м можно узнать Блин я вот в каком-то файле именно уже что-то про про что там Я не помню в в этой директории но я не помню В какой именно уже там под директории с Не ну там у тебя получается прок потом у
03:50
