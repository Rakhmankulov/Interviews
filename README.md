...
## Мои вопросы
  ...
  Какие карьерные перспективы могут ждать сотрудника вашего отдела? 
...
**cgroups** отвечают за изоляцию ресурсов
  cgroups - это функция ядра Linux, которая ограничивает, учитывает и изолирует использование ресурсов 
  (CPU, память, дисковый ввод/вывод, сеть) для набора процессов.
  cgroups - Control Groups:
  1. Ограничение ресурсов - можно определить сколько CPU или памяти будет использовать конкретный процесс
  2. Приоритезация – cgroups позволяют определить какой процесс в рамках одной cgroup более приоритетен для использования ресурсов
  3. Отчетность - лимиты ресурсов мониторятся и сообщаются на уровне cgroup
  4. Управление - можно изменить статус (frozen, stopped, restarted) всех
  процессов cgroup одной командой

###Dockerd полный путь старта контейнера:
  создается процесс 
  делается срез файловой системы == чрут (cgroups - нарезка ресурса, пробрасываются порты)
