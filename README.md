...
Это моя выжимка того что должен знать DevOps/LLMOps. Обратите внимание что вопросы раскрыты кратко, если тема Вам непонятна - следует в нее углубиться и изучить. Пулл-реквесты приветствуются. 
...
- [Ответы на вопросы](#%D0%BE%D1%82%D0%B2%D0%B5%D1%82%D1%8B-%D0%BD%D0%B0-%D0%B2%D0%BE%D0%BF%D1%80%D0%BE%D1%81%D1%8B)
    ...
    - [Сети](#%D1%81%D0%B5%D1%82%D0%B8)
        ...
        - [Назовите все уровни модели OSI](#%D0%BD%D0%B0%D0%B7%D0%BE%D0%B2%D0%B8%D1%82%D0%B5-%D0%B2%D1%81%D0%B5-%D1%83%D1%80%D0%BE%D0%B2%D0%BD%D0%B8-%D0%BC%D0%BE%D0%B4%D0%B5%D0%BB%D0%B8-osi)
...
- **Ответ:**
    ...
    **TCP** – транспортный протокол (L4 OSI) передачи данных в сетях TCP/IP, предварительно устанавливающий соединение с сетью (SYN > SYN-ACK > ACK) и гарантирующий доставку данных. Используется для передачи таких данных как *электронная почта, файлы, сообщения*. При определении потери пакетов будет выполнен перезапрос потерянных пакетов. При нарушении порядка доставки пакетов в TCP их порядок будет восстановлен.
...
- **Ответ:**
  ...
  IPv4-адрес - это 32 бита. Формула для расчета количества IP-адресов на основе префикса CIDR:
  $$N = 2^{(32 - P)} - 2$$
...
**На клиенте:**
  ...
  * **CONNECTION TIMEDOUT** - Пакеты уходят в пустоту. Две главные причины:
    * Сетевой экран (на сервере или по пути к нему) Молча сбрасывает (**Drop**) все пакеты.
    * Указанный IP-адрес сервера вообще не существует или выключен.
    ...
    * /etc/hosts
      * DNS провайдера
        * Корневые DNS-серверы
          * DNS-серверы зоны (.com, .org, .net, .ru)
...
Контейнеры не включают в себя ядро операционной системы — они используют ядро хоста, что делает их лёгкими и быстрыми по сравнению с виртуальными машинами.  
...
> Контроллер - это многопоточный системный демон, написанный на Go, который работает над согласованием текущего состояния системы `Kubernetes` с желаемым состоянием. Полезно думать о кубелете как о контроллере! Он запрашивает `Pods` из `kube-apiserver` использует механизм `List-Watch` и `Field Selectors` непосредственно при подписке на события API. Запрос выглядит примерно как `metadata.name=[node-name]`. Фильтрация происходит на стороне `kube-apiserver`, и `kubelet` изначально получает данные только о «своих» подах. Получив этот список, он обнаруживает новые добавления, сравнивая их с собственным внутренним кэшем, и начинает синхронизировать состояние, если имеются какие-либо расхождения.  
...
### Kubectl apply manifest - что происходит после этой команды (коротко)?  
  - валидация манифеста на стороне `kubectl` (client-side, с k8s 1.22 валидируется только синтаксис) и затем на `kube-apiserver` (server-side);  
  - `kubectl` идет в `kube-apiserver` на мастер-ноду, проходя при этом аутентификацию, авторизацию и admission control;  
  - `kube-apiserver` десериализует манифест и записывает объект в `etcd`, kube-apiserver — единственный компонент, который пишет напрямую в `etcd`;  
  ...
  - на воркер-ноде `kubelet` синхронизирует состояние подов через `CRI` (Container Runtime Interface) — `containerd`, `cri-o`; 
  - сначала `CRI` создает сетевой неймспейс (песочницу/sandbox), затем зовет `CNI` для настройки сети и выдачи IP-адреса, затем создаются контейнеры приложения, и только после старта контейнера запускается `PostStart lifecycle hook` (если он настроен) 
...
![Three Networks](https://github.com/Rakhmankulov/Interviews/blob/main/images/Three-Networks.jpg)
...
- **`NodePort`**: Сервис доступен на одном и том же порту на каждой ноде кластера. Например, если ты настроил NodePort на порт 30000, то можешь обратиться на `192.168.1.10:30000` или `192.168.1.11:30000`, и трафик попадёт на поды сервиса. Под капотом Kubernetes всё равно создаёт ClusterIP, но дополнительно прокидывает трафик на узлах.
  > По умолчанию Kubernetes разрешает выделять порты для NodePort только в строго определенном диапазоне: 30000–32767. 
...
* **plays/tasks/** — тут импортировались все роли через:
  - import_role: { ww.kubernetes.utilities }
